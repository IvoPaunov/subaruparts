export const getPartImgsUrls = (imgs) => imgs.map(img => process.env.PUBLIC_URL + '/static/images/parts/' + img)

/*
{
    name: '',
    oem: [''],
    oem_similar: [''],
    info_url: '',
    price: '',
    currency: 'BGN',
    description: '',
    images: getPartImgsUrls(['']),
    availability: 1,
},
*/

export const parts = [
    {
        name: 'Облицовка за под - предна',
        oem: ['95011AJ360VH'],
        oem_similar: [''],
        info_url: 'http://www.catcar.info/subaru/?lang=en&l=%3D',
        price: '150',
        currency: 'BGN',
        description: 'Добре запазена облицовка за пода, почистена е и не е наводнявана, не мирише на мухъл. Единствената забележка е, че има протъркано по педала на съединителя',
        images: getPartImgsUrls(['95011AJ360VH-1.jpg', '95011AJ360VH-2.jpg', '95011AJ360VH-3.jpg', '95011AJ360VH-4.jpg']),
        availability: 1,

    },
    {
        name: 'Декорация на фар за мъгла (спортен пакет) лаво',
        oem: ['57731AJ530'],
        oem_similar: [''],
        info_url: 'http://www.catcar.info/subaru/?lang=en&l=%3D',
        price: '35',
        currency: 'BGN',
        description: 'Има лека дракстина, която като се почисти няма да личи',
        images: getPartImgsUrls(['57731AJ530-1.jpg']),
        availability: 1,
    }
]